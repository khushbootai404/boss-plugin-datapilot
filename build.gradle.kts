...
dependencies {
    if (useLocalDependencies) {
        compileOnly(files("$bossPluginApiPath/build/libs/boss-plugin-api-1.0.91.jar"))
    } else {
...
tasks.register<Jar>("buildPluginJar") {
    archiveFileName.set("boss-plugin-datapilot-${version}.jar")
    duplicatesStrategy = DuplicatesStrategy.EXCLUDE
    manifest {
        attributes(
            "Implementation-Title" to "BOSS Data Pilot Plugin",
            "Implementation-Version" to version,
            "Main-Class" to "ai.rever.boss.plugin.dynamic.datapilot.DataPilotDynamicPlugin",
        )
    }
